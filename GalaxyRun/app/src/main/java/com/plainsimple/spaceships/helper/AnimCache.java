...
// TODO: NEEDS SIGNIFICANT RE-WORK
public class AnimCache { // todo: use enums?

    private Context appContext;
    private BitmapCache bitmapCache;

    public AnimCache(Context context, BitmapCache bitmapCache) {
        appContext = context;
    ...
    public SpriteAnimation get(BitmapID key) throws IllegalArgumentException {
        try {
            // convert key to string of params
            String anim_params = keyToString(key);
            ...
            return new SpriteAnimation(bitmapCache.getData(key), frame_speeds, loop);
        } catch (Exception e) {
    ...
    private String keyToString(BitmapID key) throws IllegalArgumentException {
        switch (key) {
            case SPACESHIP_MOVE: {
                return appContext.getString(R.string.SPACESHIP_MOVE);
            }
            case SPACESHIP_EXPLODE: {
                return appContext.getString(R.string.SPACESHIP_EXPLODE);
            }
            case SPACESHIP_FIRE: {
                return appContext.getString(R.string.SPACESHIP_FIRE_ROCKET);
            }
            case COIN_SPIN: {
                return appContext.getString(R.string.COIN_SPIN);
            }
            case ROCKET_MOVE: {
                return appContext.getString(R.string.ROCKET_MOVE);
            }
            case PROJECTILE_EXPLODE: {
                return appContext.getString(R.string.PROJECTILE_EXPLODE);
            }
            case EXPLOSION_1: {
                return appContext.getString(R.string.EXPLOSION_1);
            }
            default: {
