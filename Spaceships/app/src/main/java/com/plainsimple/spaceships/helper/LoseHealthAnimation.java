...
public class LoseHealthAnimation {
        ...
        // sprite coordinates. Adds the animation's DrawParams to the given list.
        public void updateAndDraw(float spriteX, float spriteY, List<DrawParams> spriteParams) {
            if (frameCounter == NUM_FRAMES) { // reset
                ...
                offsetX += frameDx;
                offsetY += frameDy;
                spriteParams.add(new DrawText(healthLost,
                        spriteX + offsetX, spriteY + offsetY, TEXT_COLOR, TEXT_SIZE));
            }
        }
